...
const Paymenthistory = () => {
  ...
  useEffect(() => {
    if (user && user.email) {
      fetch(
        `https://assiment-12-server.vercel.app/Paymenthistory?email=${user.email}`
      )
        .then((res) => res.json())
        .then((data) => {
                  ...
                  <td>
                    <div className="avatar">
                      <div className="mask mask-squircle w-12 h-12">
                        <img
                          src={item.alldata.singlecart.img}
                          alt="Avatar Tailwind CSS Component"
                        />
                      </div>
                    </div>
                  </td>
                  <td>{item.alldata.singlecart.name}</td>
                  <td className="text-center">
                    {item.alldata.singlecart.price}
                  </td>
                  <th>
